...
def generalized_box_iou(boxes1, boxes2):
    ...
    # if len(boxes1.shape) == 3 and len(boxes2.shape) == 3:
    #     b, n, _ = boxes1.shape
    #     boxes1 = boxes1.view(-1,4)
    #     boxes2 = boxes2.view(-1,4)
    try:
        assert (boxes1[:, 2:] >= boxes1[:, :2]).all()  # (boxes1[:, :, 2:] >= boxes1[:, :, :2]).all() or
    except:
        print(boxes1)
        assert False
    assert (boxes2[:, 2:] >= boxes2[:, :2]).all()  # (boxes2[:, :, 2:] >= boxes2[:, ::2]).all() or
