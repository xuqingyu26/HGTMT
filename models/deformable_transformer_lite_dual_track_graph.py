...
import numpy as np
from torch_geometric.nn import GATConv, GraphConv, GCNConv, AGNNConv, EdgeConv, HGTConv
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        for _ in range(2):  # number of gnn
    ...
    @staticmethod
    def build_edge_index_sparse(pc, pd, box_length: int, fm_width: int, fm_height: int):
        centers = (pc.clone() * torch.tensor([[fm_width, fm_height]]).to(pc.device))
        d_center = (pd.clone() * torch.tensor([[fm_width, fm_height]]).to(pc.device))
    ...
    def forward(self, src_r, pre_src_r, src_i, pre_src_i, pre_cts_r=None, pre_cts_i=None, no_pre_cts=False,
                pre_memories_r=None,
                pre_memories_i=None, masks_flatten=None):
        ...
        for stage in range(4):
            ...
            if stage == 0:
                ...
                gather_feat_r = F.grid_sample(pre_memory_r, (2.0 * pre_sample_r - 1.0).unsqueeze(1),
                                              mode='bilinear', padding_mode='zeros', align_corners=False)[:, :, 0,
                                :].transpose(1, 2)
                gather_feat_i = F.grid_sample(pre_memory_i, (2.0 * pre_sample_i - 1.0).unsqueeze(1),
                                              mode='bilinear', padding_mode='zeros', align_corners=False)[:, :, 0,
                                :].transpose(1, 2)
                ...
                # todo further check
                if not no_pre_cts:
                    centers = torch.cat([pre_sample_r, pre_sample_i], dim=1)
                    gather_feat = torch.cat([gather_feat_r, gather_feat_i], dim=1)
                    n1 = pre_sample_r.shape[1]
                    n2 = pre_sample_i.shape[1]
                    n3 = h * w
                    ...
                    for i in range(B):
                        ...
                        edge_index2 = self.build_edge_index_sparse(pre_sample_r[i], pre_sample_i[i], 10,
                        ...
                        graph_nodes_r = torch.cat((gather_feat[i], pre_memory_r[i].reshape(C, -1).T.contiguous()),
                                                  dim=0)
                        graph_nodes_i = torch.cat((gather_feat[i], pre_memory_i[i].reshape(C, -1).T.contiguous()),
                                                  dim=0)
                        ...
                        gather_index = offset + torch.arange(len(gather_feat[i]))
                        gather_inds.append(gather_index)
                    ...
                    gather_inds = torch.cat(gather_inds)
                    ...
                    for gnn in self.gnn:
                        if self.training:
                            ...
                            gnn_out_r, (edge_index_r, alpha_r) = gnn(gnn_feat_r, graph_r.edge_index,
                                                                     return_attention_weights=True)
                            gnn_out_i, (edge_index_i, alpha_i) = gnn(gnn_feat_i, graph_i.edge_index,
                                                                     return_attention_weights=True)
                    ...
                    center_feats_r = gnn_feat_r[center_inds].reshape(B, H, W, C).permute(0, 3, 1, 2).contiguous()
                    center_feats_i = gnn_feat_i[center_inds].reshape(B, H, W, C).permute(0, 3, 1, 2).contiguous()

                    gather_feat_f = gnn_feat_r[gather_inds].reshape(B, n1 + n2, hd).contiguous()
                    gather_feat_r = gather_feat_f[:, :n1, :]
                    gather_feat_i = gather_feat_f[:, n1:, :]
        ...
        pre_query_embed_r = self.my_forward_ffn(gather_feat_r)
        pre_query_embed_i = self.my_forward_ffn(gather_feat_i)
