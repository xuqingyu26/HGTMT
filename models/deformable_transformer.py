...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 hidden_dim=256,
                 pretrained="/home/user/PycharmProjects/MOT_Project/[RGBT-HGT]/TransCenter_rgbt/model_zoo/pvtv2_backbone/pvt_v2_b2.pth",
                 linear=False,
                 half=True):
        super().__init__()
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024, dropout=0.1, activation="relu", n_levels=4, n_heads=8, n_points=4):
        ...
        # cross attention
        self.cross_attn = MSDeformAttn(d_model,  n_heads, n_levels, n_points, batch_first=True
                                       )
    ...
    def forward(self, pre_tgt, pre_query_pos, src_spatial_shapes,
        ...
        #                                                   src_padding_mask))
        pre_tgt = pre_tgt + self.dropout1(self.cross_attn(query=self.with_pos_embed(pre_tgt, pre_query_pos),
                                                          value=src,
                                                          key_padding_mask=src_padding_mask,
                                                          reference_points=pre_ref_pts,
