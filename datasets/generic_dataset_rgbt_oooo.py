...
class GenericDataset_rgbt(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if opt.tracking:
            ...
            pre_image_r, pre_image_i, pre_anns_r, pre_anns_i, frame_dist, pre_img_id_r, pre_img_id_i = self._load_pre_data(
                [img_info_r['video_id'], img_info_i['video_id']], img_info_i['frame_id'],
        ...
        assert len(pre_cts_i) == len(pre_track_ids_i)
        for k, (pre_ct_r, pre_track_id_r, pre_ct_i, pre_track_id_i) in enumerate(
                zip(pre_cts_r, pre_track_ids_r, pre_cts_i, pre_track_ids_i)):
    ...
    def _load_pre_data(self, video_id, frame_id, sensor_id=1):
        img_infos_r = self.video_to_images_r[video_id[0]]
        img_infos_i = self.video_to_images_i[video_id[1]]
    ...
    def _get_input(self, img_r, img_i, anns_r=None, anns_i=None, flip=0, M=None, a=None):
        ...
        inp_i = (inp_i - self.normMean) / self.normstd
