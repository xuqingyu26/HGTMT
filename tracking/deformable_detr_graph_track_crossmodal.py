...
from deformable_transformer_lite_dual_track_graph3_gnnloss import build_deforamble_transformer
import torch
...
from losses.losses import FastFocalLoss, RegWeightedL1Loss, loss_boxes, loss_boxes_sdiou, SparseRegWeightedL1Loss
...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, transformer, num_classes, output_shape, args, half=True):
        ...
        # for _ in range(3):
        #     self.gnn.append(GraphConv(64, 64))
    ...
    def forward(self, samples_r: NestedTensor, samples_i, pre_samples_r: NestedTensor, pre_samples_i, pre_cts_r: Tensor,
        ...
        merged_hs_r, merged_hs_i, [gather_feat_r, gather_feat_i], [edge_r, edge_i] = self.transformer(samples_r, pre_samples_r, samples_i, pre_samples_i,
                                                        pre_cts_r, pre_cts_i, no_pre_cts)

        out_i = self.regress(merged_hs_i)
        out_r = self.regress(merged_hs_r)

        return [out_r, out_i], [gather_feat_r, gather_feat_i], [edge_r, edge_i]
    ...
    def regress(self, merged_hs):
        ...
        for layer_lvl in range(len(hs)):
            ...
            # pre_hs[layer_lvl] = self.ida_up(pre_hs[layer_lvl], 0, len(pre_hs[layer_lvl]))[-1]
                       # torch.cuda.synchronize()
            ...
            outputs_reid_pre.append(pre_hs[layer_lvl])
