...
class Tracker:
    ...
    def __init__(self, obj_detect, reid_network, flownet, tracker_cfg, postprocessor=None,
        ...
        self.u_tracks_r = []
        self.u_tracks_i = []
        self.CLASSES = ('ship', 'car', 'cyclist', 'pedestrian', 'bus', 'drone', 'plane')
    ...
    # def dual_tracks_dets_matching(self, raw_dets_r, raw_scores_r, pre2cur_cts_r, pos_r, gather_feat_r,
    #                               reid_cts_r,
    #                               raw_det_feats_r, raw_cls_r, raw_dets_i, raw_scores_i,
    #                               pre2cur_cts_i, pos_i, gather_feat_i, reid_cts_i,
    #                               raw_det_feats_i, raw_cls_i, hs_m_r=None, hs_m_i=None):
    #
    #     return None
    ...
    def dual_tracks_dets_matching_tracking(self, raw_dets, raw_scores, pre2cur_cts, pos, gather_feat, reid_cts,
        ...
        if raw_dets.nelement() > 0:
            ...
            if len(dets) > 0:
                ...
                if not edge == None:
                    sim_mat = edge_remain.permute(1, 0)
                else:
                    sim_mat = 1
                # matching with gIOU
                ##################
                ...
                a, b, c = self.linear_assignment(1-sim_mat.cpu().numpy(),
                                                 thresh=0.4)

                match_rows, match_cols = linear_sum_assignment(1-sim_mat.cpu().numpy())
                ...
                color_iou = cm.jet(scores_keep[matches[:, 1]].cpu().numpy())
                text_iou = [
                    'Iou',
                    'Keypoints: {}:{}'.format(len(kpts0), len(kpts1)),
                    'Matches: {}'.format(len(mkpts0_iou))
                ]
                mkpts0_gnn = kpts0[match_rows]
                mkpts1_gnn = kpts1[match_cols]
                color_gnn = cm.jet(scores_keep[match_cols].cpu().numpy())
                text_gnn = [
                    'GNN',
                    'Keypoints: {}:{}'.format(len(kpts0), len(kpts1)),
                    'Matches: {}'.format(len(mkpts0_gnn))
                ]
                ...
                if isV:
                    last_frame = self.last_image_r.cpu().numpy()
                    frame = self.image_r.cpu().numpy()
                else:
                    last_frame = self.last_image_i.cpu().numpy()[:, :, 0]
                    frame = self.image_i.cpu().numpy()[:, :, 0]
                ...
                if len(matches[:, 0]) > 0:
                    pc = pre_cls.cpu().numpy().copy()
                    pc[matches[:, 0]] = 0
                    cls = cls_keep.cpu().numpy().copy()
                    cls[matches[:, 1]] = 0
                    ...
                    frame = imshow_det_bboxes(
                        ...
                        font_size=0,
                        show=False,
                        out_file=f'frame_iou.png')

                    # out = make_matching_plot_fast(
                    #     cv2.cvtColor(last_frame, cv2.COLOR_BGR2GRAY), cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY),
                    #     kpts0.cpu().numpy(), kpts1.cpu().numpy(), mkpts0.cpu().numpy(),
                    #     mkpts1.cpu().numpy(), color, text,
                    ...
                    out = make_matching_plot_fast(
                        ...
                        path=None, show_keypoints=True)
                    if isV:
                        cv2.imshow('SuperGlue matches_r', out)
                        cv2.imwrite('SuperGlue_matches_iou_r.png', out)
                    else:
                        cv2.imshow('SuperGlue matches_i', out)
                        cv2.imwrite('SuperGlue_matches_iou_i.png', out)
                ...
                if len(match_rows) > 0:
                    ...
                    out = make_matching_plot_fast(
                        last_frame, frame,
                        kpts0.cpu().numpy(), kpts1.cpu().numpy(), mkpts0_gnn.cpu().numpy(),
                        mkpts1_gnn.cpu().numpy(), color_gnn, text_gnn,
                        path=None, show_keypoints=True)
                    if isV:
                        cv2.imshow('SuperGlue matches_r', out)
                        cv2.imwrite('SuperGlue_matches_gnn_r.png', out)
                    else:
                        cv2.imshow('SuperGlue matches_i', out)
                        cv2.imwrite('SuperGlue_matches_gnn_i.png', out)
            ...
            if isV:
                if len(self.inactive_tracks_r) > 0:
                    ...
                    for t in remove_inactive:
                        ...
                        if t in self.u_tracks_r:
                            self.u_tracks_r.remove(t)
                ...
                if len(self.inactive_tracks_i) > 0:
                    ...
                    for t in remove_inactive:
                        ...
                        if t in self.u_tracks_i:
                            self.u_tracks_i.remove(t)
    ...
    def dual_step_reidV3_pre_tracking_vit(self, blob):
        ...
        self.image_r = blob['img_r']
        self.image_i = blob['img_i']
        ...
        # except:
        #     gt_bboxes_r = torch.zeros([0, 4])
        #     label_r = torch.ones(0, dtype=np.int)
        #
        # label_i_txt = os.path.join(
        #     '/home/user/PycharmProjects/MOT_Project/mmtracking/data/MOT_01/labels_with_ids/test',
        #     blob['video_name'], 'img1', blob['frame_name'].replace('.jpg', '.txt'))
        ...
        # except:
        #     gt_bboxes_i = torch.zeros([0, 4])
        #     label_i = torch.ones(0, dtype=np.int)
        #
        ...
        # except:
        #     print('error happend when cv2.write')
        ...
        # Predict tracks #
        ##################
        # if len(self.tracks_r) and len(self.tracks_i):
        #     self.dual_tracks_dets_matching(
        #         raw_dets_r=det_pos_r, raw_scores_r=det_scores_r, pre2cur_cts_r=pre2cur_cts_r, pos_r=mypos_r,
        #         gather_feat_r=gather_feat_r, reid_cts_r=reid_cts_r, raw_det_feats_r=reid_feat_r, raw_cls_r=det_cls_r,
        #         raw_dets_i=det_pos_i, raw_scores_i=det_scores_i, pre2cur_cts_i=pre2cur_cts_i, pos_i=mypos_i,
        #         gather_feat_i=gather_feat_i, reid_cts_i=reid_cts_i, raw_det_feats_i=reid_feat_i, raw_cls_i=det_cls_i,
        #         hs_m_r=outputs[0]['reid_mem'], hs_m_i=outputs[1]['reid_mem']
        #     )
        ...
        ################################################################################################
        # Find possible positions for untracked objects
        ...
        self.pre_sample_r = self.sample_r
        self.pre_sample_i = self.sample_i
        self.last_image_r = self.image_r
        self.last_image_i = self.image_i
...
class Track(object):
    ...
    def bbox_to_ct(self):
        ct = self.pos.clone()
        ct[:, 0] = (self.pos[:, 2] + self.pos[:, 0]) / 2
        ct[:, 1] = (self.pos[:, 3] + self.pos[:, 1]) / 2
        return ct[:, :2]
